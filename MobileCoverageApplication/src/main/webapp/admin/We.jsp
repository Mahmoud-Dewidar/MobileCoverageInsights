...
<div class="vodafone" style="width: 26%; ">
    <img src="images/We.png" width="50" height="50" style="border-radius: 15px;">
    <h1
            style="font-family:  'Dancing Script','Poppins'; font-size: 47px;font-weight:600; color: #5204b1; padding-left: 10px;">
        We Telecom Egypt</h1>
</div>
...
<div class="handset">
    ...
    <div class="handsetInfo">
        <h3 class="info" style="color: rgb(4, 92, 164);">SAMSUNG: <%= conn.noOfPhoneType("we","samsung") %></h3>
        <h3 class="info" style="color: rgb(3, 105, 3); ">OPPO: <%= conn.noOfPhoneType("we","oppo") %></h3>
        <h3 class="info" style="color: rgb(154, 4, 4);">HUAWEI: <%= conn.noOfPhoneType("we","huawei") %></h3>
        <h3 class="info" style="color: rgb(229, 151, 6);">REALME: <%= conn.noOfPhoneType("we","realme") %></h3>
        <h3 class="info" style="color: rgb(183, 164, 0);">XIAOMI: <%= conn.noOfPhoneType("we","xiaomi") %></h3>
        ...
        <div class="col-xl-3 col-xxl-3 col-sm-4">
            <div class="card bg-warning invoice-card">
                <div class="card-body d-flex">
                    ...
                    <div>
                        <h2 class="text-white invoice-num"><%=conn.noOfCitiesByOperator("we")%></h2>
        ...
        <div class="col-xl-3 col-xxl-3 col-sm-6">
            <div class="card bg-success invoice-card">
                <div class="card-body d-flex">
                    ...
                    <div>
                        <h2 class="text-white invoice-num"><%=conn.noOfCellsByOperator("we")%></h2>
...
<script>
    function initMap() {
        ...
        <% List<Point> pointsList = conn.getPointsByOperator("we");
        String lat;
        String lon;
        ...
        <% List<Point> pointsListt = conn.getPointsByOperator("Telecom Egypt");
        String latt;
        String lonn;
        double signalStrengthh;
        String cityy;
        if (pointsListt != null) {
            for (Point point : pointsListt) {
                if(point.getLatitude() != null && point.getLatitude() != null && point.getSignalStrength() != null && point.getCity() != null){
                 latt = point.getLatitude();
                 lonn = point.getLongitude();
                 signalStrengthh = point.getSignalStrength();
                 cityy = point.getCity();
                }
                                else{
                                    continue;
        ...
        %>
        points.push({lat: <%= latt %>, lon: <%= lonn %>, signalStrength: <%= signalStrengthh %>, city: "<%= cityy %>"});
        <% } } %>
        ...
        points.forEach(function (point) {
            // Create the marker
            var marker = L.marker([point.lon, point.lat],{ icon: violetIcon }).addTo(map);
...
<script>
    ...
    var chart = new Chart(ctxx, {
        ...
        data: {
            ...
            datasets: [{
                data: [<%=conn.getPowerPercentage("we","100","70")%>, <%=conn.getPowerPercentage("we","70","40")%>, <%=conn.getPowerPercentage("we","40","0")%>],
